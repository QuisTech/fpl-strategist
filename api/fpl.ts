  ...
  try {
    ...
    const scoreAll = (players: FPLPlayer[]) => players.map(p => {
      ...
      return {
        ...
        next_fixtures: data.fixtures.filter(f => !f.finished && (f.team_h === p.team || f.team_a === p.team)).slice(0, 3).map(f => ({
    ...
    if (url?.includes('/api/recommendations') || url?.includes('/api/fpl/recommendations')) {
    ...
    } else if (url?.includes('/api/sync') || url?.includes('/api/fpl/sync')) {
      ...
      const transfers = squad.sort((a, b) => a.score - b.score).slice(0, 5).map(pOut => {
        ...
        return { out: pOut, in: pIn, scoreJump: pIn.score - pOut.score };
